...
export default function JobSlider() {
  ...
  const [currentSlide, setCurrentSlide] = useState(0);
  const [isLoading, setIsLoading] = useState(true);
  const [jobsPerSlide, setJobsPerSlide] = useState(3); // Responsive job count
  ...
  // Handle responsive jobs per slide
  useEffect(() => {
    const handleResize = () => {
  ...
  }, [totalSlides, jobs.length, jobsPerSlide]);
    ...
    <div className="w-full bg-white py-8 md:py-16 min-h-[300px] md:min-h-[400px]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {jobs.length === 0 ? (
          ...
          <div className="relative">
            {/* Navigation Arrows - Hidden on mobile when only 1 slide */}
            {totalSlides > 1 && (
              <>
                <button
                  onClick={prevSlide}
                  className="hidden sm:block absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white rounded-full p-2 shadow-md hover:bg-gray-100 transition-colors"
                  aria-label="Previous slide"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                ...
                <button
                  onClick={nextSlide}
                  className="hidden sm:block absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white rounded-full p-2 shadow-md hover:bg-gray-100 transition-colors"
                  aria-label="Next slide"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
              ...
              >
                {Array.from({ length: totalSlides }).map((_, slideIndex) => {
                  ...
                  return (
                    <div
                      key={slideIndex}
                      className="w-full flex-shrink-0 flex justify-center gap-4 sm:gap-6 px-2 sm:px-0"
                    >
                      {slideJobs.map((job) => (
                        <Link
                          key={job._id}
                          href={`/jobs/${job._id}`}
                          className="block group cursor-pointer w-full sm:w-56 md:w-64 flex-shrink-0"
                        >
                          <div className="relative w-full aspect-[3/4] rounded-xl overflow-hidden bg-gray-100 flex items-center justify-center">
                            <img
                              src={job.imageUrl}
                              alt={job.title}
